"""
Sprint B - Phase 3: Populate Embeddings (FIXED)
Compute and cache embeddings for all CVs and jobs in the database
Uses correct column names: skills_technical, skills_soft, required_skills
"""
...
def parse_skills(skills_text: str) -> list:
    """Parse comma-separated skills from TEXT field"""
    if not skills_text:
        return []
...
def populate_cv_embeddings():
    ...
    try:
        ...
        # 3. Fetch all CVs with skills
        print("\n2️⃣  Fetching CV data...")
        fetch_sql = text("""
        SELECT cv_id, skills_technical, skills_soft
        FROM cvs
        WHERE (skills_technical IS NOT NULL AND skills_technical != '') 
           OR (skills_soft IS NOT NULL AND skills_soft != '')
        ORDER BY cv_id;
        """)
        ...
        for row in result:
            ...
            technical_skills = parse_skills(row[1])
            soft_skills = parse_skills(row[2])
            all_skills = technical_skills + soft_skills
            
            if all_skills:  # Only add if there are skills
                cv_data.append({
                    "cv_id": row[0],
                    "skills": all_skills
                })
...
def populate_job_embeddings():
    ...
    try:
        ...
        for row in result:
            ...
            required_skills = parse_skills(row[1])
            preferred_skills = parse_skills(row[2])
...
def main():
    ...
    print("\n🚀 SPRINT B - PHASE 3: POPULATE ALL EMBEDDINGS (FIXED)")
    print("=" * 60)
    print()
